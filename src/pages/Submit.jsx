...
export default function Submit() {
	...
	const handleSubmit = async (e) => {
		...
		if (!form.name || !form.neighborhood) {
			setError("Venue name and neighborhood are required.");
			return;
		}

		const filledSpecials = form.specials.filter((s) => s.trim() !== "");
		if (filledSpecials.length === 0) {
		...
		return (
			<div style={{
				...
				<div style={{ textAlign: "center", maxWidth: "360px" }}>
					...
					<h2 style={{ color: "#F5C842", fontSize: "22px", fontWeight: 800, marginBottom: "8px" }}>
						Deal submitted
					</h2>
					<p style={{ color: "#888", fontSize: "14px", marginBottom: "28px" }}>
						We'll review it and get it on the map within 24 hours. Thanks for adding to Austin's happy hour scene.
					</p>
					<button
						onClick={() => navigate("/")}
						style={{
	...
	return (
		<div style={{
			...
			<div style={{ maxWidth: "560px", margin: "0 auto" }}>
				<h1 style={{ color: "#F5C842", fontSize: "24px", fontWeight: 800, marginBottom: "6px" }}>
					Submit a Deal
				</h1>
				<p style={{ color: "#888", fontSize: "14px", marginBottom: "32px" }}>
					Know a happy hour Austin needs to know about? Add it to the map.
				</p>
				...
				<form onSubmit={handleSubmit}>
								...
								{DAYS.map((day) => (
									<button
										...
										style={{
											...
											background: form.days.includes(day) ? "#D4A017" : "transparent",
											color: form.days.includes(day) ? "#000" : "#888",
											borderColor: form.days.includes(day) ? "#D4A017" : "rgba(212,160,23,0.25)",
										}}
									>
										{day}
									</button>
					...
					<Section label="The Specials">
						<p style={{ color: "#888", fontSize: "12px", marginBottom: "12px" }}>
							Add up to 3 specials. Be specific -- "$3 Lone Stars" beats "cheap beer".
						</p>
							...
							{["🍺", "🍸", "🎸", "🤘", "🦌", "🍹", "🥃", "🍕", "🎵", "🌮"].map((icon) => (
								<button
									...
									style={{
										...
										fontSize: "20px", cursor: "pointer", border: "1px solid",
										background: form.icon === icon ? "rgba(212,160,23,0.15)" : "transparent",
										borderColor: form.icon === icon ? "#D4A017" : "rgba(212,160,23,0.2)",
									}}
								>
									{icon}
								</button>
					...
					<button
						...
						disabled={loading}
						style={{
							width: "100%", padding: "14px", background: "#D4A017",
